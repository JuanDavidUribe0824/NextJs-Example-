    ...
    return (
        <Layout footer =  {false} dark>
            <div className="row justify-content-center">
                <div className="cold-md offset-md">
                    <div className="card card-body text-center">
                        <h1>{user.name}</h1>
                        <img src ={user.avatar_url} alt='' className="rounded mx-auto d-block"/>
                        <p>{user.bio}</p>
...
export async function getServerSideProps() {
    const res = await fetch('https://api.github.com/users/JuanDavidUribe0824')
